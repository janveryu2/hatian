          ...
          >
            ...
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-3">
                <span className="text-3xl p-2.5 rounded-2xl bg-accent-primary-soft text-accent-primary">
                  {bill.category?.icon || "📋"}
                </span>
                <div>
            ...
            {/* Status Banner */}
            {bill.isProvisional ? (
              <div className="mb-4 px-3.5 py-2 rounded-xl bg-accent-amber-soft border border-accent-amber/30 flex items-center gap-2 text-caption text-accent-amber font-medium">
                <span>⏳</span>
                <span>
              ...
              <div className="mb-4 px-3.5 py-1.5 rounded-xl bg-accent-teal-soft border border-accent-teal/20 flex items-center gap-2 text-caption text-accent-teal font-medium">
                <span>✓</span>
            ...
            {!isPayer && myShare && (
              <div
                className={`p-4 rounded-2xl mb-4 border transition-all ${
                  isMyShareConfirmed
                    ? "bg-accent-teal-soft border-accent-teal/30"
                    : isMySharePaid
                    ? "bg-accent-amber-soft border-accent-amber/30"
                    : isMyShareAcknowledged
                    ? "bg-accent-primary-soft border-accent-primary-border"
                    : "bg-accent-coral-soft border-accent-coral/30"
                }`}
              >
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="shrink-0 flex items-center gap-2">
                    {isMyShareConfirmed ? (
                      ...
                      <span className="px-3.5 py-1.5 rounded-xl bg-accent-amber-soft text-accent-amber text-body-sm font-semibold">
                        {t("bills.sentPendingConfirm")}
                      </span>
                    ) : isMyShareAcknowledged ? (
                      <div className="flex items-center gap-2">
                        <span className="text-caption px-2.5 py-1 rounded-xl bg-accent-primary-soft text-accent-primary font-semibold">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          disabled={processingShareId === myShare.id}
                          className="px-3 py-2 rounded-xl bg-bg-surface border border-border-hairline text-text-secondary hover:text-text-primary text-caption font-semibold transition-colors"
                        >
                        ...
                        <button
                          ...
                          onClick={() => handleMarkPaid(myShare.id)}
                          disabled={processingShareId === myShare.id}
                          className="btn-primary py-2 px-3.5 text-body-sm font-bold flex items-center gap-1"
                        >
            ...
            <div className="grid grid-cols-2 gap-3 mb-5">
              <div className="p-3.5 rounded-xl bg-bg-surface border border-border-hairline">
                    ...
                    >
                      <div className="flex items-center justify-between gap-3">
                        <div className="flex items-center gap-3 min-w-0">
                          <div className="w-8 h-8 rounded-xl bg-accent-primary-soft flex items-center justify-center text-caption font-bold text-accent-primary uppercase overflow-hidden shrink-0">
                          ...
                          <div className="min-w-0">
                            <p className="text-body-sm font-semibold text-text-primary truncate flex items-center gap-1">
                              {share.userName}
                              {isShareOwner && (
                                <span className="text-[10px] px-1.5 py-0.2 rounded-full bg-accent-primary-soft text-accent-primary">
                                  {t("common.you")}
                                </span>
                              )}
                            </p>
                        ...
                        <div className="shrink-0">
                          {isSharePayer ? (
                            ...
                            isPayer || isAdmin ? (
                              ...
                              <span className="px-2.5 py-1 rounded-full bg-accent-amber-soft text-accent-amber text-caption font-medium">
                                ⏳ {t("settle.pendingConfirmPill")}
                              </span>
                            )
                          ) : isAcknowledged ? (
                            <span className="px-2.5 py-1 rounded-full bg-accent-primary-soft text-accent-primary text-caption font-medium">
                              {t("bills.willPayLaterBadge")}
                            </span>
                          ) : isShareOwner ? (
                            <div className="flex items-center gap-1.5">
                              <button
                                ...
                                disabled={isBusy}
                                className="px-2 py-1 rounded-lg bg-bg-card border border-border-hairline text-caption text-text-secondary hover:text-text-primary font-medium transition-colors"
                              >
                                {t("bills.payLaterButton")}
                              </button>
                              <button
                            ...
                            <span className="px-2.5 py-1 rounded-full bg-accent-coral-soft text-accent-coral text-caption font-medium">
                      ...
                      <div className="pt-2 border-t border-border-hairline flex items-center justify-between">
                        <div className="flex items-center gap-1.5">
                          ...
                          {share.is_days_confirmed ? (
                            <span className="text-[10px] px-1.5 py-0.2 rounded-md bg-accent-teal-soft text-accent-teal font-medium">
                              {t("bills.confirmedDaysBadge")}
                            </span>
                          ) : (
                            <span className="text-[10px] px-1.5 py-0.2 rounded-md bg-accent-amber-soft text-accent-amber font-medium">
                        ...
                        {canEditDays ? (
                          <div className="flex items-center gap-1.5 bg-bg-card border border-border-hairline rounded-xl p-0.5">
                            <button
                              type="button"
                          ...
                          <span className="px-2.5 py-1 rounded-lg bg-bg-card border border-border-hairline font-mono text-caption font-bold text-text-secondary">
                            {share.is_days_confirmed
              ...
              <div className="pt-3 border-t border-border-subtle">
                {!showDeleteConfirm ? (
                  <button
                    ...
                    className="w-full py-2 text-body-sm font-medium text-accent-coral hover:bg-accent-coral-soft rounded-xl transition-colors text-center"
                  >
                    {t("bills.deleteBill")}
                  </button>
                ) : (
                  <div className="p-3.5 rounded-xl bg-accent-coral-soft border border-accent-coral/30 text-center space-y-2">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        disabled={isDeleting}
                        className="flex-1 px-3 py-1.5 rounded-xl bg-accent-coral text-white font-medium text-body-sm"
                      >
